...
* **A licensed installation, on `POST /channels` only.** `PrivacySettings.UseAnonymousURLs` behind
  ...
  neither is gated.
* **`RestrictDirectMessage = "team"`** — [D-235]. Needs a store method this port does not have and a
  plugin decision it cannot make.
* **A view-restricted caller** — [D-236], the same wall `GET /users/by_auth_data` already hits. Both

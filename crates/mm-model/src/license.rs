...
mod tier_go_parity {
    ...
    /// `LicenseToLicenseTier` and the three gates over every SKU the corpus names — including
    /// the legacy `E10`/`E20`, the empty string, and the case and whitespace variants that a
    /// normalising comparison would wrongly accept. A miss is `0`, below every rung.
    #[test]
    fn the_tier_ladder_and_the_three_gates_match_go() {
        let oracle = oracle();
        let cases = oracle["tiers"].as_array().unwrap();
        assert!(cases.len() >= 14);
        for case in cases {
            let sku = case["sku"].as_str().unwrap();
            let license = License {
            ...
            assert_eq!(
                license_tier(sku),
                case["tier"].as_i64().unwrap(),
                "tier of {sku:?}"
            );
            assert_eq!(
                minimum_professional_license(Some(&license)),
                case["professional"].as_bool().unwrap(),
                "professional gate on {sku:?}"
            );
            assert_eq!(
                minimum_enterprise_license(Some(&license)),
                case["enterprise"].as_bool().unwrap(),
                "enterprise gate on {sku:?}"
            );
            assert_eq!(
                minimum_enterprise_advanced_license(Some(&license)),
                case["advanced"].as_bool().unwrap(),
                "advanced gate on {sku:?}"
            );
        }

        let nil = &oracle["minimum_nil"];
        assert_eq!(
            minimum_professional_license(None),
            nil["professional"].as_bool().unwrap()
        );
        assert_eq!(
            minimum_enterprise_license(None),
            nil["enterprise"].as_bool().unwrap()
        );
        assert_eq!(
            minimum_enterprise_advanced_license(None),
            nil["advanced"].as_bool().unwrap()
        );
    }

    /// The eight predicates handlers branch on, over licences built to sit on each side of every
    /// comparison they make: the exact trial durations and their off-by-ones, the sanctioned
    /// bounds (named backwards in Go, tested as "outside the band"), the cloud-preview hour, and
    /// the feature-or-tier disjunctions with the feature off.
    #[test]
    fn the_licence_predicates_match_go() {
        let oracle = oracle();
        let cases = oracle["predicates"].as_array().unwrap();
        assert!(cases.len() >= 21);
        for case in cases {
            let name = case["name"].as_str().unwrap();
            let license: License = serde_json::from_value(case["license"].clone())
                .unwrap_or_else(|e| panic!("{name}: {e}"));
            let check = |field: &str, actual: bool| {
                assert_eq!(actual, case[field].as_bool().unwrap(), "{name}: {field}");
            };
            check("is_trial_license", license.is_trial_license());
            check("is_sanctioned_trial", license.is_sanctioned_trial());
            check("is_cloud", license.is_cloud());
            check("is_cloud_preview", license.is_cloud_preview());
            check("has_shared_channels", license.has_shared_channels());
            check(
                "has_remote_cluster_service",
                license.has_remote_cluster_service(),
            );
            check(
                "has_enterprise_marketplace_plugins",
                license.has_enterprise_marketplace_plugins(),
            );
            check("is_mattermost_entry", license.is_mattermost_entry());
            let minimum = &case["minimum"];
            assert_eq!(
                minimum_professional_license(Some(&license)),
                minimum["professional"].as_bool().unwrap(),
                "{name}: minimum.professional"
            );
            assert_eq!(
                minimum_enterprise_license(Some(&license)),
                minimum["enterprise"].as_bool().unwrap(),
                "{name}: minimum.enterprise"
            );
            assert_eq!(
                minimum_enterprise_advanced_license(Some(&license)),
                minimum["advanced"].as_bool().unwrap(),
                "{name}: minimum.advanced"
            );
        }
    }
}

...
use super::super::common::local_socket::{
    ...
    over_socket, post_over_socket, rust_socket, sockets_enabled,
};
...
/// **The one pair in these three files that is *not* ported still reaches Go.** `patchRole` is
/// unregistered here, so it falls to the router fallback. (`convertBotToUser`, which this test
/// used to pair it with, is served since 2026-09-14 — `parity::local_misc` compares it.)
#[tokio::test]

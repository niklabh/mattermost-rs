...
mod tests {
    ...
    /// The 1028×1028 refusal carries **both** `MaxWidth` and `MaxHeight`, and they are two keys.
    ///
    /// # Why this cannot be a parity test
    ///
    /// `AppError.params` is `params map[string]any` in Go — **unexported, no tag** — and
    /// `#[serde(skip)]` here, so it never reaches the wire on either server. It exists only to
    /// interpolate `message`, which [D-092] already tolerates as differing until i18n lands. So a
    /// cross-server body comparison is structurally blind to these two keys, and a mutation
    /// renaming one of them to the other survived the whole parity suite until this test existed.
    ///
    /// The bytes are the oracle's own `png_header_1029x1028`, so the input is one Go itself
    /// measured at 1029×1028 rather than one transcribed here. The values are transcribed from
    /// app/emoji.go:36 for the reason above: Go will not hand them out.
    #[tokio::test]
    async fn the_too_large_refusal_names_both_dimensions() {
        use base64::Engine;
        ...
        let case = oracle["decode_config"]
            ...
            .expect("an array")
            .iter()
            .find(|case| case["name"].as_str() == Some("png_header_1029x1028"))
            .expect("the corpus carries an over-wide PNG");
        assert_eq!(case["width"].as_i64(), Some(1029), "Go measured it at 1029");
        ...
        let err = app
            ...
            .await
            .expect_err("1029 is over the limit");

        let crate::post::PrepareError::App(err) = err else {
            panic!("this is a refusal, not a forward");
        };
        assert_eq!(err.id, "api.emoji.upload.large_image.too_large.app_error");
        assert_eq!(err.status_code, 400);
        assert_eq!(err.where_, "uploadEmojiImage");

        let params = err.params.as_ref().expect("both limits are interpolated");
        assert_eq!(
            params.len(),
            2,
            "two keys, not one — `MaxWidth` and `MaxHeight` are separate parameters"
        );
        assert_eq!(
            params.get("MaxWidth"),
            Some(&serde_json::Value::from(super::MAX_EMOJI_ORIGINAL_WIDTH))
        );
        assert_eq!(
            params.get("MaxHeight"),
            Some(&serde_json::Value::from(super::MAX_EMOJI_ORIGINAL_HEIGHT))
        );
    }

    /// The height half of the same check, which the width half would otherwise shadow.
    ///
    /// `width > MAX || height > MAX` short-circuits, so every over-wide input leaves the height
    /// comparison unevaluated. An input that is **inside** the width limit and outside the height
    /// one is the only thing that reaches it — and the oracle has one.
    #[tokio::test]
    async fn an_over_tall_image_is_refused_by_the_height_half() {
        use base64::Engine;
        ...
        let case = oracle["decode_config"]
            ...
            .expect("an array")
            .iter()
            .find(|case| case["name"].as_str() == Some("png_header_1028x1029"))
            .expect("the corpus carries an over-tall PNG");
        assert_eq!(case["width"].as_i64(), Some(1028), "inside the width limit");
        assert_eq!(
            case["height"].as_i64(),
            Some(1029),
            "outside the height one"
        );
        let bytes = base64::engine::general_purpose::STANDARD
            .decode(case["bytes_base64"].as_str().expect("base64"))
            .expect("valid base64");
        ...
        let err = app
            ...
            .await
            .expect_err("1029 tall is over the limit");
        let crate::post::PrepareError::App(err) = err else {
            panic!("this is a refusal, not a forward");
        };
        assert_eq!(err.id, "api.emoji.upload.large_image.too_large.app_error");

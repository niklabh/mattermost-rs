...
## D-157 · `the_in_team_etag_has_no_membership_deletion_condition` ties on a same-millisecond clock

**Status** OPEN · **Severity** flaky test · **Raised** 2026-08-22 (phase 2, `autocompleteUsers`)

`crates/mm-store/tests/db_user_profile_lists.rs:494` asserts that two consecutive calls to
`get_etag_for_profiles` differ, on the grounds that the query's failure path falls back to
`CurrentVersion.GetMillis()` and "never matches a second call". It matches whenever both calls
land in the same millisecond, which on this machine is roughly one run in three:

```
assertion `left != right` failed: the millisecond fallback must not be mistaken for a stable etag
  left: "11.11.0.1787378925501"
 right: "11.11.0.1787378925501"
```

Reproduced on a **clean tree** with no local changes, so it is not caused by the route that found
it.

# The flake takes the shared Go server down, which is the actual severity

Each test in that file seeds rows and purges them at the end, and **an assertion panics past the
trailing purge**. Its `insert_user` sets only eight columns, so the six rows it leaves behind have
`password`, `authservice`, `emailverified`, `allowmarketing`, `props`, `notifyprops`,
`lastpasswordupdate`, `failedattempts`, `locale`, `timezone`, `mfaactive` and `mfasecret` all
SQL NULL — and Go's `model.User` cannot scan any of them. From that moment
**`GET /api/v4/users` returns 500 on the Go server** for every worktree sharing the database,
and every suite that reads an unfiltered user list fails with it. Measured: after one flake,
`parity_users_list` failed four tests, all of them on Go's own 500, and clearing the six rows
restored it immediately. This is the same trap the roles session recorded ("a test fixture can
take the shared stack down"), reached by a different route.

**To pay off**, two independent fixes and both are wanted:

1. Make the assertion about the *property* rather than the value — that the fallback is
   clock-derived and therefore not a function of the data — or sleep past a millisecond boundary
   between the two reads.
2. Make that file's `insert_user` spell out every column backing a non-pointer Go field, the way
   `db_user_search.rs` and the roles fixture do, so that a panicked-past purge leaves rows the Go
   server can still read.

Not fixed here because the file belongs to another route's suite and three worktrees are editing
this tree concurrently; the orphaned rows were cleared by hand so the shared stack came back.
